...
> **⭐⭐ 重大修正（2026-07-15 晚，实测推翻「PC 抓不了」）**：此前判定「PC 自动化被反爬拦死」的所有测试,用的都是**全新、未登录**的浏览器。经用户授权,用**已登录淘宝(dreamer199506)的真实 Chrome 配置**(CDP 接管)重测国内深链 `market.m.taobao.com/app/trip/rx-flight-eco/pages/listing`(北京→杭州)——**页面完整渲染出航班,`mtop.trip.flight.listingsearch` 返回 `SUCCESS` + 10 条航班,截图确认价格日历/筛选栏/航班卡片俱全**。
>
> **结论**：**PC 自动化抓取在登录态下完全可行**,之前的「必须安卓真机」是误判(根因是没登录,不是反爬/自动化检测/机房 IP)。**架构转向 PC 优先**:本地 Playwright/CDP 驱动一个**持久化已登录的 Chrome** 搜索并拦截 listingsearch → 自建前端渲染卡片。原 Flask+前端方案回归可用(数据源＝驱动登录态 Chrome,而非纯 requests)。安卓 App 退为次要(同样机制:真实 WebView 登录态拦截)。登录态存持久化 Chrome 用户目录(需放**稳定目录**,勿放会被清理的 scratchpad)。
...
## 10. 数据源转向 Google Flights（2026-07-16，因飞猪自动化不稳定）

**动机**：飞猪登录态浏览器抓取虽可用，但**慢（每次起浏览器 10–30s）、飘（反自动化，国际页 rx-iflight-eco 需容器内导航、易碎）、需登录**。用户要求更稳更快的数据源（如谷歌）。

**验证结论（都已实测）**：
- **Google Flights 单次 HTTP 即可抓取**：用 `fast-flights` 构造 `tfs` 查询 URL，`requests.get` 直接取 HTML（`hl=zh-CN&curr=CNY`），**无登录、无浏览器**，约 **3–4 秒**。
- 境外站，正好走**路由器代理（Vultr 境外 IP）畅通**——与飞猪需国内 IP 互补。
- 数据在航班 `aria-label`（中文、含人民币价格），解析出：价格、航司、直飞/中转+中转地、起降机场/时间、跨天、总时长。国内/国际、直飞/中转均覆盖。
- 复用同一归一化结构（sources/parse.py 的 schema），**前端零改动**。

**⚠️ 价格口径差异（重要）**：Google 显示航司挂牌起价那类，**不含中国 OTA 折扣舱** → **国内航线价格明显高于飞猪**（实测 北京→杭州 Google ¥2560 vs 飞猪 ¥465）；**国际航线较有竞争力**（上海→东京 Google ¥1653、上海→新加坡 ¥850）。用户以国际为主，故 Google 为合适的默认源。

**架构落地**：
- 新增 [sources/google.py](sources/google.py)：`search(dep_code, arr_code, date)` → 抓取+解析为归一化航班。
- [app.py](app.py) `/api/search?source=google|fliggy`，**默认 `google`**（env `CHEAPESTFLIGHT_SOURCE` 可改）；谷歌源不加载浏览器、无需登录。飞猪源惰性加载（`_get_fliggy`）保留为可选（国内特价时用）。
- 前端加「数据源」下拉（谷歌航班 / 飞猪）；谷歌源隐藏登录横幅。
- 实测：`python app.py`（免 profile）→ 上海→东京 9 航班/4.3s、北京→杭州 27 航班/3.8s，卡片/排序/筛选正确。

**Google 源已知取舍**：无航班号、无航站楼（aria-label 未提供）；个别航司中转地名解析为空（layover 文案变体，影响小）；依赖 aria-label 文案格式（谷歌改版需更新正则，但无反爬、比飞猪稳得多）。

**下一步**：往返/收藏/保存 filter 等功能在 Google 源上叠加（更稳）；飞猪源作为「国内特价深挖」可选项保留。
